        ...
        <div className="flex items-center gap-6 xl:gap-10">
          ...
          <div className="flex items-center gap-3">
            ...
            {status === "authenticated" && session?.user ? (
              ...
              // The Kinsen navy → expanding-teal-circle hover system is
              // shared (see kinsen-glow-button.tsx) with the Login page's
              // main "Σύνδεση" submit button — the only two controls that
              // use it. Everything else here (asChild/Link, `hidden
              // min-w-[140px] sm:inline-flex`) is this call site's own, so
              // every other `variant="primary"` button on the site keeps
              // its plain hover treatment untouched.
              <Button
                asChild
                variant="primary"
                size="sm"
                className={cn(KINSEN_GLOW_SURFACE_CLASSNAME, "hidden min-w-[140px] rounded-md sm:inline-flex")}
              >
                <Link href="/login">
                  <KinsenGlowOverlay />
